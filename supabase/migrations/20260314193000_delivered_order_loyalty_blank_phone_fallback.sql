-- ============================================================================
-- Delivered order loyalty blank-phone fallback fix
-- - treats blank customer_phone/contact_phone values as missing
-- - preserves phone fallback when only contact_phone is populated
-- ============================================================================
...
BEGIN
    ...
    v_order_phone_digits := regexp_replace(
        COALESCE(
            ...
            NULLIF(BTRIM(v_order.contact_phone), ''),
            ''
        ),
        '[^0-9]',
        '',
