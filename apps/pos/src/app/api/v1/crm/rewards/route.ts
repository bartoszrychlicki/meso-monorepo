...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await listRewards(createServiceClient(), {
      page,
      perPage,
      search,
    ...
    const message = error instanceof Error ? error.message : 'Nie udało się pobrać nagród';
...
export async function POST(request: NextRequest) {
  ...
  try {
    const reward = await createReward(createServiceClient(), validation.data);
    return apiCreated(reward);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Nie udało się utworzyć nagrody';
    return apiError('INTERNAL_ERROR', message, 500);
  }
}
