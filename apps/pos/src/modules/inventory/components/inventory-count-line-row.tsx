...
export function InventoryCountLineRow({
  ...
  const differenceLabel = useMemo(() => {
    ...
    if (difference === 0) {
      return `0 ${line.stock_item_unit}`;
    }

    const sign = difference > 0 ? '+' : '-';
    return `${sign}${formatQuantity(Math.abs(difference))} ${line.stock_item_unit}`;
  }, [difference, line.stock_item_unit]);
    ...
    <tr className={rowClassName}>
      ...
      <td className="px-3 py-3 align-top">
        <Select
          ...
          disabled={isReadonly}
        >
          <SelectTrigger className="h-9 w-[150px]" data-field="inventory-category">
